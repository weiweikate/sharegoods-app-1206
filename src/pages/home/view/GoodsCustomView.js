...
export default class GoodsCustomView extends React.Component {
                ...
                <TouchableWithoutFeedback onPress={() => {
                    ...
                    <View style={[style, style2]}>
                        ...
                        <View style={{ flex: 1, marginHorizontal: autoSizeWidth(10) }}>
                            {this.renderTitle(data, item)}
                            {this.rederDetail(data, item)}
                            {commissionVisible ? <MRText style={{
                                fontSize: autoSizeWidth(12),
                                color: DesignRule.mainColor
                            }}>{item.minPriceY ? '佣金¥' + item.minPriceY : ''}</MRText> : null}
                            <View style={{ flex: 1 }}/>
                            <View style={{ flexDirection: 'row' }}>
                                <MRText style={styles.tip}>{'拼店价'}</MRText>
                                {priceHasInvalidVisible ?
                                    <MRText
                                        style={[styles.oldPrice, { marginLeft: 5 }]}>{'¥' + item.originalPrice}</MRText>
                                    : null}
                            </View>
                            <MRText style={{
                                fontSize: autoSizeWidth(12),
    ...
    renderImage(data, item, width) {
        ...
        return (
            ...
            >
                {mainPicBorderVisible ? <ImageLoader source={{ uri: mainPicBorderUrl }} style={{
                        position: 'absolute',
                        top: 0,
                        left: 0,
                        height: width,
    ...
    renderCommission(data, item) {
        let {
            commissionVisible, layout, priceNameVisible,
            priceNameAlias
        } = data;
        if (!user.isLogin) {
            commissionVisible = false;
        }
        if (!commissionVisible && !priceNameVisible) {
            return null;
        }
        let style = {
        ...
        return <View style={style}>
            {priceNameVisible ? <View style={{ flexDirection: 'row' }}>
                ...
                : null}
            {commissionVisible ?
                <MRText style={textStyle}>{item.minPriceY ? '佣金¥' + item.minPriceY : ''}</MRText> : null}
        </View>;
    }
    ...
    renderOldPrice(data, item) {
        ...
        switch (layout) {
            ...
            case 3:
            case 8:
                return <MRText style={[styles.oldPrice, {
                    marginLeft: 5,
                    height: autoSizeWidth(14)
                }]}>{'￥' + item.originalPrice}</MRText>;
    ...
    renderBtn(data, item) {
        ...
        switch (data.layout) {
            ...
            case 3:
                ...
                buyBtnStyle = {
                    borderRadius: autoSizeWidth(12), overflow: 'hidden',
    ...
    render() {
        ...
        return (
            <View style={{
                height: (height - autoSizeWidth(15)),
...
export function GoodsCustomViewGetItemHeight(data) {
    ...
    let height = 0;
    let {
        buyButtonVisible, buyButtonType,
        titleVisible, subtitleVisible, priceNameVisible,
        priceHasInvalidVisible, commissionVisible
    } = data;
    switch (data.layout) {
        case  1 :
        ...
        case  2 :
            ...
            if (subtitleVisible) {
                height += ScreenUtils.autoSizeWidth(33 / 2);
            }//detail
            height += ScreenUtils.autoSizeWidth(5);//间距
            if (commissionVisible || priceNameVisible) {
        ...
        case  3 :
            ...
            if (subtitleVisible) {
                height += ScreenUtils.autoSizeWidth(15);
            }//detail
            if (priceNameVisible) {
                height += ScreenUtils.autoSizeWidth(14);
            }//拼店
