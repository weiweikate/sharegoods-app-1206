...
const { fa_huo, ji_su, qi_tian, zheng_ping, afterSaleLimitImg } = res.service;
...
export default class DetailHeaderServiceModal extends Component {
    ...
    render() {
        const { sevenDayReturn, weekendDelivery, groupActivity } = this.state.pData;
        ...
        let sectionListData = [
            ...
            {
                headerTittle: afterSaleLimit ? '仅支持换货' : `${sevenDayReturn ? '' : '不支持'}7天无理由退换货`,
                headerImg: afterSaleLimit ? afterSaleLimitImg : qi_tian,
                data: [{ content: afterSaleLimit ? '消费者自商品购买后，仅支持换货，不支持退款及退货退款。' : `${sevenDayReturn ? '收到商品之日7天（含）内，可在线申请退货服务（部分食品、贴身衣物等特殊商品除外）' : '食品、贴身衣物、兑换、秒杀、经验翻倍等特殊商品，无质量问题不支持退换货'}` }]
            },
            {
