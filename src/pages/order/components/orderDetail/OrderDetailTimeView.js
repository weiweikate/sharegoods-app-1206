    ...
    render() {
        return (
            <View style={{ backgroundColor: 'white',paddingTop:px2dp(10) }}>
                ...
                {StringUtils.isNoEmpty(orderDetailModel.warehouseOrderDTOList[0].payTime) && this.props.status > 1 ?
                    <UIText value={'平台付款时间：' + DateUtils.getFormatDate(orderDetailModel.warehouseOrderDTOList[0].payTime / 1000)}
                            style={styles.textGoodsDownStyle}/> : null}
                {StringUtils.isEmpty(orderDetailModel.warehouseOrderDTOList[0].cancelTime) ? null :
                ...
                {StringUtils.isNoEmpty(this.props.payTime) && (this.props.payType % 2 === 0) && this.props.status > 1 ?
                    <UIText value={'三方付款时间：' + DateUtils.getFormatDate(this.props.payTime / 1000)}
                            style={styles.textGoodsDownStyle}/> : null}
                {StringUtils.isNoEmpty(orderDetailModel.warehouseOrderDTOList[0].outTradeNo) && (this.props.payType % 2 === 0) ?
                ...
                {StringUtils.isEmpty(orderDetailModel.warehouseOrderDTOList[0].finishTime) ? null :
                    <UIText
                        value={'完成时间：' + DateUtils.getFormatDate(orderDetailModel.warehouseOrderDTOList[0].deliverTime ? orderDetailModel.warehouseOrderDTOList[0].deliverTime / 1000 : orderDetailModel.warehouseOrderDTOList[0].finishTime / 1000)}
                        style={styles.textOrderDownStyle}/>}
                {this.renderWideLine()}
            </View>
        );
