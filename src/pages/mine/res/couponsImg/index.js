const couponsImg = {
    coupons_no_data: require('./coupons_no_data.png'),
    youhuiquan_bg_unUsedBg_ex: require('./youhuiquan_bg_nor.png'),
    youhuiquan_bg_unUsedBg: require('./youhuiquan_bggrey_nor.png'),
    youhuiquan_bg_unUsedBg_exd: require('./youhuiquan_bg_shouhui.png'),
    youhuiquan_bg_usedBg_ex: require('./youhuiquan_bgline_zhihui.png'),
    youhuiquan_bg_usedBg: require('./youhuiquan_bg_zhihui.png'),
    youhuiquan_bg_usedBg_exd: require('./youhuiquan_bg_zhihui_shouhui.png'),
    youhuiquan_icon_jia_nor: require('./youhuiquan_icon_jia_nor.png'),
    youhuiquan_icon_jian_nor: require('./youhuiquan_icon_jian_nor.png'),
    youhuiquan_icon_topArrow: require('./youhuiquan_icon_zhankai_nor.png'),
    youhuiquan_icon_topArrowed: require('./youhuiquan_icon_shouhui_nor.png'),
    youhuiquan_icon_smallUp: require('./tongyong_icon_jinru_blackup.png'),
    youhuiquan_icon_smallDown: require('./tongyong_icon_zhankai_black.png'),
};
export default couponsImg;
