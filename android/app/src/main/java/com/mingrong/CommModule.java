...
import com.mingrong.bean.AreaListResponse;
import com.mingrong.bean.CityPickerBean;
import com.mingrong.bean.NetCommonParamsBean;
import com.mingrong.event.CaptureScreenImageEvent;
import com.mingrong.event.LoadingDialogEvent;
import com.mingrong.utils.Utils;
import com.liuxingyu.bean.CityBean;
import com.liuxingyu.bean.DistricBean;
import com.liuxingyu.bean.ProvinceBean;
import com.liuxingyu.pickerview.OptionsPickerView;
...
//    private TimePickerView timePicker;
    ...
    public static ArrayList<ArrayList<CityBean>> options2Items = new ArrayList<ArrayList<CityBean>>();
    public static ArrayList<ArrayList<ArrayList<DistricBean>>> options3Items = new ArrayList<ArrayList<ArrayList<DistricBean>>>();
...
//    }
    ...
    public void setCityPicker(String str) {
        List<AreaListResponse> list =(new Gson()).fromJson(str, new TypeToken<List<AreaListResponse>>() {}.getType());
        CommModule.options1Items.clear();CommModule.options2Items.clear();CommModule.options3Items.clear();
        if (list.size()!=0){
            for (int i=0;i<list.size();i++){
                ...
                for (int j=0;j<list.get(i).getValue().size();j++){
                    ...
                    for (int k=0;k<list.get(i).getValue().get(j).getValue().size();k++){
                        option3Item_temp1.add(new DistricBean(list.get(i).getValue().get(j).getValue().get(k).getCode(),list.get(i).getValue().get(j).getValue().get(k).getName()));
                    }
                    //如果区为空
                    ...
                    option3Iteems_temp2.add(option3Item_temp1);
                    option2Items_temp.add(new CityBean(list.get(i).getValue().get(j).getCode(),list.get(i).getValue().get(j).getName()));
                }
                //如果市为空
                ...
                CommModule.options1Items.add(new ProvinceBean(list.get(i).getCode(),list.get(i).getName()));//添加省
                CommModule.options2Items.add(option2Items_temp);//添加市
                CommModule.options3Items.add(option3Iteems_temp2);//添加区
            }
        }
...
//                    });
                    ...
                    pvOptions = new OptionsPickerView(mContext.getCurrentActivity());
                    // 初始化三个列表数据
...
//        pwOptions.setLabels("省", "市", "区");
                    ...
                    pvOptions.setOnoptionsSelectListener(new OptionsPickerView.OnOptionsSelectListener() {
                        ...
                        public void onOptionsSelect(int options1, int option2, int options3) {
                            ...
                            cityPickerBean.setCityName(options2Items.get(options1).get(option2).getName());
                            cityPickerBean.setCityId(options2Items.get(options1).get(option2).getId());
                            cityPickerBean.setDistricName(options3Items.get(options1).get(option2).get(options3).getName());
                            cityPickerBean.setDistricId(options3Items.get(options1).get(option2).get(options3).getId());
