...
import static com.reactnative.ivpusic.imagepicker.picture.lib.imaging.core.clip.ClipRatioConfig.RatioNone;
...
public interface IMGClip {
    ...
    byte[] CLIP_CORNERS = {0x8, 0x8, 0x9, 0x8, 0x6, 0x8, 0x4, 0x8, 0x4, 0x8, 0x4, 0x1, 0x4, 0xA, 0x4, 0x8, 0x4, 0x4, 0x6, 0x4, 0x9, 0x4, 0x8, 0x4, 0x8, 0x4, 0x8, 0x6, 0x8, 0x9, 0x8, 0x8};

    enum Anchor {
        LEFT(1), RIGHT(2), TOP(4), BOTTOM(8), LEFT_TOP(5), RIGHT_TOP(6), LEFT_BOTTOM(9), RIGHT_BOTTOM(10);
        ...
        public void move(RectF win, RectF frame, float dx, float dy) {
            ...
            float centerX = (theFrame[0] + theFrame[1]) / 2;
            float centerY = (theFrame[2] + theFrame[3]) / 2;
            ...
            for (int i = 0; i < 4; i++) {
                if (((1 << i) & v) != 0) {
                    ...
                    theFrame[i] = pn * revise(pn * (theFrame[i] + dxy[i & 2]), pn * maxFrame[i], pn * minFrame[i + PN[i & 1]]);
                }
            }

            if (ClipRatioConfig.CURRENT_CLIP_RATIO_CONFIG == RatioNone) {
                //自由裁剪
                ...
                if (v == 4 || v == 8) {
                    ...
                    float width = height * ClipRatioConfig.CURRENT_CLIP_RATIO_CONFIG;
                    frame.set(centerX - width / 2, theFrame[2], centerX + width / 2, theFrame[3]);
                } else if (v == 1 || v == 2) {
                    float width = theFrame[1] - theFrame[0];
                    float height = width / ClipRatioConfig.CURRENT_CLIP_RATIO_CONFIG;
                    frame.set(theFrame[0], centerY - height / 2, theFrame[1], centerY + height / 2);
                } else {
                    float height = theFrame[3] - theFrame[2];
                    float width = theFrame[1] - theFrame[0];
                    float diagonal = (float) Math.sqrt(Math.pow(height, 2) + Math.pow(width, 2));
                    double angle = Math.atan(1 / ClipRatioConfig.CURRENT_CLIP_RATIO_CONFIG);
                    double sin = Math.sin(angle);
                    double cos = Math.cos(angle);
                    height = (float) (diagonal * sin);
                    width = (float) (diagonal * cos);
                    if (v == 10) {
                        //右下角裁剪
                        frame.set(theFrame[0], theFrame[2], theFrame[0] + width, theFrame[2] + height);
                    } else if (v == 9) {
                        //左小角裁剪
                        frame.set(theFrame[1] - width, theFrame[2], theFrame[1], theFrame[2] + height);
                    }else if(v == 5){
                        frame.set(theFrame[1] - width, theFrame[3]-height, theFrame[1], theFrame[3]);
                    }else if(v == 6){
                        frame.set(theFrame[0], theFrame[3]-height, theFrame[0]+width, theFrame[3]);
                    }
                }
            }
        }
