...
- (void)startRunning {
  if (!_session.isRunning) {//没有运行，开始运行
    [_session startRunning];
    if (![_currentController.view.layer.sublayers containsObject:_videoPreviewLayer]) {
      [_currentController.view.layer insertSublayer:_videoPreviewLayer atIndex:0];
    }
  }
}

- (void)stopRunning {
  if (_session.isRunning) {//运行中，结束运行
