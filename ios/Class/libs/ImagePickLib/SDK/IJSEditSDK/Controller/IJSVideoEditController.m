...
//    };
   ...
   //裁剪控制器
    ...
    [self _hiddenVideoDrawingViewSubView:YES]; //隐藏画板
    [self _videoDrawToolSubViewUnableUserInteractionEnabled:NO];  // 不允许交互
    self.cutHodelView.hidden = NO;
    self.placeholderToolView.hidden = YES;
    
    self.naviView.hidden = YES;
    self.toolView.hidden = YES;  // 隐藏工具条
    [self.view bringSubviewToFront:self.cutHodelView];
    [self resetTrimView];  //重新加预览条
...
-(void)resetTrimView
{
    ...
    IJSVideoTrimView *trimView = [[IJSVideoTrimView alloc] initWithFrame:CGRectMake(0, 0, JSScreenWidth, IJSVideoSecondCuttrimViewHeight * 0.7) minCutTime:self.minCutTime ?: 5 maxCutTime:self.maxCutTime ?: 30 assetDuration:CMTimeGetSeconds([self.resultAvasset duration]) avAsset:self.resultAvasset];
