...
- (void)sendWithImage:(UIImage *)image videoPath:(NSString *)videoPath{
  ...
  if (image) {
    if (self.finshBlock) {
      self.finshBlock(image, nil, hyf_img);
    }
  }else if(videoPath && videoPath.length){
    if(self.finshBlock){
      self.finshBlock(nil, videoPath, hyf_video);
    }
  }
...
- (SGRecordSuccessPreview *)preview{
    if (!_preview) {
      ...
      [_preview setEditBlock:^(UIImage *image,NSString * videoPath) {
        STRONGSELF
        if (image) {
          [strongSelf editImage:image videoPath:videoPath];
        }
      }];
